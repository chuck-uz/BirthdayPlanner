...
def _operator_telegram_id(prompt: AdminBirthdayPrompt) -> int | None:
    if prompt.prompt_recipient_telegram_id is not None:
        return int(prompt.prompt_recipient_telegram_id)
    return None


def _callback_operator_allowed(from_id: int, prompt: AdminBirthdayPrompt) -> bool:
    if prompt.prompt_recipient_telegram_id is not None:
...
async def ensure_admin_prompt_for_birthday(
    ...
    *,
    ...
    """Создать/обновить подсказку подписчику: «создайте группу сами»."""
    settings = get_settings()
    if trigger_subscriber is None:
        return
    ...
    if existing is not None:
        if (
            ...
            if existing.state == STATE_PROMPT_SENT and existing.admin_prompt_message_id is None:
                mid = await telegram_send_message_message_id(
                    recipient_tid,
                    _admin_prompt_text(name, days_left, count),
...
async def handle_telegram_callback_query(session: AsyncSession, cq: dict[str, Any]) -> None:
    ...
    if not _callback_operator_allowed(from_tid, prompt):
    ...
    op_chat = _operator_telegram_id(prompt)
